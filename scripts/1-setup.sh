...
if [[ ${DESKTOP_ENV} != server ]]; then
	sed -n '/'${INSTALL_TYPE}'/q;p' "${HOME}/ArchLAbS/pkg-files/pacman-pkgs.txt" | while read line; do
...
if ! source "${HOME}/ArchLAbS/configs/setup.conf"; then
	# Loop through user input until the user gives a valid username
	while true; do
		read -rep "Please enter username:" username
	...
	echo "username=${username,,}" >>"${HOME}/ArchLAbS/configs/setup.conf"

	# Set Password
	read -rep "Please enter password:" password
	echo "password=${password,,}" >>"${HOME}/ArchLAbS/configs/setup.conf"

	# Loop through user input until the user gives a valid hostname, but allow the user to force save
	while true; do
		read -rep "Please name your machine:" name_of_machine
		# hostname regex (!!couldn't find spec for computer name!!)
		if [[ ${name_of_machine,,} =~ ^[a-z][a-z0-9_.-]{0,62}[a-z0-9]$ ]]; then
			break
		fi
		# if validation fails allow the user to force saving of the hostname
		read -rep "Hostname doesn't seem correct. Do you still want to save it? (y/n)" force
		if [[ ${force,,} == "y" ]]; then
			break
		fi
	done

	echo "NAME_OF_MACHINE=${name_of_machine,,}" >>"${HOME}/ArchLAbS/configs/setup.conf"
fi
...
if [[ "$(whoami)" == "root" ]]; then
	...
	echo "'ArchLAbS' copied to home directory"
